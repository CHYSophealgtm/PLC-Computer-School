...
export default function App() {
  ...
  const isStudentExamReq = queryParams.get("student_exam") === "true" ||
                           queryParams.get("course_exam") === "true" ||
                           queryParams.get("exam") === "true";

  const isStudentLoginRequested = queryParams.get("parent_login") === "true" ||
                                  queryParams.get("student_login") === "true" ||
                                  queryParams.get("portal") === "parent" ||
                                  queryParams.get("portal") === "student" ||
                                  queryParams.get("portal") === "guardian" ||
                                  portalStudentId === "login" ||
                                  portalStudentId === "parent" ||
                                  portalStudentId === "guardian" ||
                                  isStudentExamReq;

  const [studentSession, setStudentSession] = useState<{ studentId: string; autoExam: boolean } | null>(null);
  const [isForcedAdminMode, setIsForcedAdminMode] = useState(false);

  // 1. Direct student portal ID query parameter
  if (portalStudentId && portalStudentId !== "login" && portalStudentId !== "parent" && portalStudentId !== "guardian" && !isAdminLogin && !isForcedAdminMode) {
    return (
      <>
        <StudentPortal 
          studentId={portalStudentId} 
          initialOpenExam={isStudentExamReq}
          onBackToLogin={() => {
            if (typeof window !== "undefined") {
              window.history.replaceState({}, '', window.location.pathname);
            }
            setIsForcedAdminMode(true);
          }}
        />
        <InstallBanner />
      </>
  ...
  // 2. Active student portal session from login form
  if (studentSession && !isForcedAdminMode && !isAdminLogin) {
    return (
      <>
        <StudentPortal 
          studentId={studentSession.studentId} 
          initialOpenExam={studentSession.autoExam}
          onBackToLogin={() => setStudentSession(null)}
        />
        <InstallBanner />
      </>
    );
  ...
  // 3. Student / Parent Login Form request route
  if (isStudentLoginRequested && !isForcedAdminMode && !isAdminLogin) {
    return (
      <>
        <StudentLoginForm 
          onLogin={(studentId, autoExam) => {
            setStudentSession({ studentId, autoExam: autoExam || isStudentExamReq });
          }}
  ...
  const isExplicitLoginReq = isAdminLogin || isStudentLoginRequested || isForcedAdminMode;
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem("plc_theme") || localStorage.getItem("plc_app_theme");
    if (savedTheme) {
      ...
      const hexVal = savedTheme.startsWith('#')
        ? savedTheme
        : THEME_PRESETS[savedTheme];
      
      if (hexVal) {
        const hexToRgb = (h: string) => {
          let cleanHex = h.trim().replace(/^#/, '');
        ...
        if (rgb) {
          const palette: Record<string, string> = {
            ...
            '100': rgbToHex(rgb.r + (255 - rgb.r) * 0.84, rgb.g + (255 - rgb.g) * 0.84, rgb.b + (255 - rgb.b) * 0.84),
            '200': rgbToHex(rgb.r + (255 - rgb.r) * 0.68, rgb.g + (255 - rgb.g) * 0.68, rgb.b + (255 - rgb.b) * 0.68),
            '300': rgbToHex(rgb.r + (255 - rgb.r) * 0.48, rgb.g + (255 - rgb.g) * 0.48, rgb.b + (255 - rgb.b) * 0.48),
            '400': rgbToHex(rgb.r + (255 - rgb.r) * 0.24, rgb.g + (255 - rgb.g) * 0.24, rgb.b + (255 - rgb.b) * 0.24),
            '500': rgbToHex(rgb.r, rgb.g, rgb.b),
            '600': rgbToHex(rgb.r * 0.86, rgb.g * 0.86, rgb.b * 0.86),
            '700': rgbToHex(rgb.r * 0.72, rgb.g * 0.72, rgb.b * 0.72),
            '800': rgbToHex(rgb.r * 0.58, rgb.g * 0.58, rgb.b * 0.58),
            '900': rgbToHex(rgb.r * 0.42, rgb.g * 0.42, rgb.b * 0.42),
            '950': rgbToHex(rgb.r * 0.25, rgb.g * 0.25, rgb.b * 0.25),
          };
          document.documentElement.setAttribute('data-theme', 'custom');
          Object.entries(palette).forEach(([s, v]) => {
            document.documentElement.style.setProperty(`--primary-${s}`, v);
          });
        }
      } else {
