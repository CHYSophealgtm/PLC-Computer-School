...
import CourseFinalExamTab from "./tabs/CourseFinalExamTab";
import PreCourseTestTab from "./tabs/PreCourseTestTab";
...
export default function Dashboard({ user, token, onLogout }: DashboardProps) {
  ...
  const customPermissions = useMemo(() => {
    if (user?.role === "ADMIN") {
      return ["Dashboard", "Analytics", "Announcements", "Students", "Courses", "Timetable", "Course Final Exam", "Pre-Course Test", "Certificates", "Teachers", "Leave", "Attendance", "QR Scan", "Attendance Display", "Finance", "Assets", "ID Card", "Credentials", "Settings"];
    }
    try {
    ...
    if (user?.role === "ACCOUNTANT") {
      return ["Dashboard", "Analytics", "Announcements", "Students", "Courses", "Timetable", "Course Final Exam", "Pre-Course Test", "Attendance", "QR Scan", "Finance"];
    }
    if (user?.role === "TEACHER") {
      return ["Dashboard", "Students", "Courses", "Timetable", "Course Final Exam", "Pre-Course Test", "Attendance", "QR Scan", "Leave"];
    }
    return null;
  }, [user, permsVersion]);

  const [uiLang, setUiLang] = useState<"en" | "kh" | "zh">(
    (localStorage.getItem("plc_lang") as "en" | "kh" | "zh") || "kh"
  );
    ...
    if (!isFullscreen && !document.fullscreenElement) {
      ...
      if (requestFs) {
        requestFs.call(docEl)
          ...
          .catch((err: any) => {
            console.warn("Native fullscreen unavailable, enabling in-app fullscreen:", err);
            setIsFullscreen(true);
          });
      } else {
        setIsFullscreen(true);
...
model User {
  ...
  const handleDeleteCourseOption = async (target: number | string) => {
    let targetVal = "";
    let updated: string[] = [];

    if (typeof target === "number") {
      targetVal = courseOptions[target] || "";
      updated = courseOptions.filter((_, i) => i !== target);
    } else {
      targetVal = target.trim();
      updated = courseOptions.filter(c => c.trim().toLowerCase() !== targetVal.toLowerCase());
    }
    ...
    // Save targetVal to local deleted list
    if (targetVal) {
      try {
        const stored = localStorage.getItem("plc_deleted_courses");
        const list: string[] = stored ? JSON.parse(stored) : [];
        if (!list.some(item => item.trim().toLowerCase() === targetVal.toLowerCase())) {
          list.push(targetVal);
          localStorage.setItem("plc_deleted_courses", JSON.stringify(list));
        }
      } catch (e) {}
    }
  ...
  const fetchTimetables = () => {
    ...
    fetch("/api/timetables", {
      ...
      .catch(err => { console.error("Error fetching timetables:", err); setTimetables([]); });
  };
  ...
  const fetchDbCourses = () => {
    ...
    fetch("/api/courses", {
      ...
      .catch(err => { console.error("Error fetching courses:", err); setDbCourses([]); });
  };
  ...
  useEffect(() => {
    ...
    if (regCourse && timetables.length > 0) {
      ...
      if (!isInitialEditLoad) {
        const matchingSchedules = timetables.filter(t => t && t.subject?.trim().toLowerCase() === regCourse.trim().toLowerCase());
  ...
  useEffect(() => {
    ...
    fetch('/api/expenses', {
      ...
      .catch(err => console.warn("Notice: Error fetching expenses:", err));
  }, [user, token]);
  ...
  const [schoolLogo, setSchoolLogo] = useState(() => {
    try { return localStorage.getItem("plc_school_logo") || ""; } catch (e) { return ""; }
  });
  const [schoolStamp, setSchoolStamp] = useState(() => {
    try { return localStorage.getItem("plc_school_stamp") || ""; } catch (e) { return ""; }
  });
  const [directorSignature, setDirectorSignature] = useState(() => {
    try { return localStorage.getItem("plc_director_signature") || ""; } catch (e) { return ""; }
  });
  const [coverImage, setCoverImage] = useState(() => {
    try { return localStorage.getItem("plc_cover_image") || "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&q=80&w=800"; } catch (e) { return "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&q=80&w=800"; }
  });
  ...
  const [khqrImage, setKhqrImage] = useState(() => {
    try { return localStorage.getItem("plc_khqr_image") || ""; } catch (e) { return ""; }
  });
  ...
  const [developerLogo, setDeveloperLogo] = useState(() => {
    try { return localStorage.getItem("plc_developer_logo") || ""; } catch (e) { return ""; }
  });
  ...
  const [headerBgColor, setHeaderBgColor] = useState<string>(() => {
    try {
      const saved = localStorage.getItem("plc_header_bg");
      return saved || "#3B82F6";
    } catch {
      return "#3B82F6";
    }
  ...
  const [sidebarBgColor, setSidebarBgColor] = useState<string>(() => {
    try {
      const saved = localStorage.getItem("plc_sidebar_bg");
      return saved || "#ffffff";
    } catch {
      return "#ffffff";
    }
  ...
  const [sidebarActiveColor, setSidebarActiveColor] = useState<string>(() => {
    try {
      const saved = localStorage.getItem("plc_sidebar_active_color");
      return saved || "#3B82F6";
    } catch {
      return "#3B82F6";
    }
  ...
  const [sidebarTextColor, setSidebarTextColor] = useState<string>(() => {
    try {
      const saved = localStorage.getItem("plc_sidebar_text_color");
      return saved || "#334155"; // Default text-slate-700
    } catch {
      return "#334155";
    }
  });
  ...
  const systemSearchModules = useMemo(() => [
    ...
    { id: "Course Final Exam", kh: "ប្រឡងបញ្ចប់វគ្គសិក្សា", en: "Course Final Exams", desc: "គ្រប់គ្រង និងបង្កើតវិញ្ញាសាប្រឡងបញ្ចប់វគ្គ", category: "ពិន្ទុ", icon: Award },
    { id: "Pre-Course Test", kh: "ប្រឡងតេស្ដ មុនវគ្គសិក្សា", en: "Pre-Course Placement Test", desc: "គ្រប់គ្រងវិញ្ញាសាតេស្ដវាស់ស្ទង់សមត្ថភាពមុនចូលរៀន", category: "ពិន្ទុ", icon: Compass },
  ...
  const globalSearchResults = useMemo(() => {
    ...
    const matchedStudents = (searchCategoryFilter === 'all' || searchCategoryFilter === 'students')
      ? (students || []).filter(s => {
          ...
          const name = (stAny.name || s.nameKh || "").toLowerCase();
          const khmerName = (stAny.khmerName || s.nameKh || "").toLowerCase();
          const nameEn = (s.nameEn || "").toLowerCase();
          const id = (s.id || "").toLowerCase();
          const phone = (stAny.phone || s.phoneNumber || s.guardianPhone || "").toLowerCase();
    ...
    const matchedTeachers = (searchCategoryFilter === 'all' || searchCategoryFilter === 'teachers')
      ? (teachers || []).filter(t => {
          if (!t) return false;
          const name = (t.name || t.nameKh || "").toLowerCase();
  ...
  useEffect(() => {
    ...
    const targetHex = appTheme.startsWith('#')
      ? appTheme
      : (THEME_PRESETS[appTheme] || '#3b82f6');

    const palette = generatePrimaryPalette(targetHex);
    if (palette) {
    ...
    try {
      ...
      localStorage.setItem("plc_app_theme", appTheme);
    } catch (e) {
      console.error(e);
    }
  }, [appTheme]);
  ...
  useEffect(() => {
    if (headerBgColor) {
      try {
        localStorage.setItem("plc_header_bg", headerBgColor);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  useEffect(() => {
    if (sidebarBgColor) {
      try {
        localStorage.setItem("plc_sidebar_bg", sidebarBgColor);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  useEffect(() => {
    if (sidebarActiveColor) {
      try {
        localStorage.setItem("plc_sidebar_active_color", sidebarActiveColor);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  useEffect(() => {
    if (sidebarTextColor) {
      try {
        localStorage.setItem("plc_sidebar_text_color", sidebarTextColor);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const [idCardNameKh, setIdCardNameKh] = useState("សុខ ចាន់ដារ៉ា");
  const [idCardNameEn, setIdCardNameEn] = useState("SOK CHANDARA");
  const [idCardIdNumber, setIdCardIdNumber] = useState("");
  const [idCardGender, setIdCardGender] = useState("Male");
  const [idCardField1, setIdCardField1] = useState("Computer Repair & Maintenance"); // Course or Specialty
  const [idCardField2, setIdCardField2] = useState("Level 1"); // Level or Phone
  const [idCardField3, setIdCardField3] = useState("ថ្ងៃចន្ទ - ថ្ងៃសុក្រ"); // Shift or Email
  const [idCardField4, setIdCardField4] = useState("08:00 - 09:00 AM"); // Hours or Join Date/Status
  ...
  const [idCardBackgroundFront, setIdCardBackgroundFront] = useState<string>(() => {
    try { return localStorage.getItem("plc_id_card_bg_front") || ""; } catch(e) { return ""; }
  });
  const [idCardBackgroundBack, setIdCardBackgroundBack] = useState<string>(() => {
    try { return localStorage.getItem("plc_id_card_bg_back") || ""; } catch(e) { return ""; }
  });
  ...
  const handleSaveIdCardBackgrounds = async () => {
    ...
    try {
      if (idCardBackgroundFront) {
        try { localStorage.setItem("plc_id_card_bg_front", idCardBackgroundFront); } catch (e) {}
      }
      if (idCardBackgroundBack) {
        try { localStorage.setItem("plc_id_card_bg_back", idCardBackgroundBack); } catch (e) {}
      }
      ...
      if (res.ok) {
        showToast("រក្សាទុកផ្ទៃខាងក្រោយកាតដោយជោគជ័យ!", "success");
      } else {
        throw new Error("Failed to save");
      }
    } catch (err) {
      console.error(err);
      showToast("មានបញ្ហាក្នុងការរក្សាទុកផ្ទៃខាងក្រោយ!", "error");
    } finally {
  ...
  const handlePrefillStudent = (student: StudentType) => {
    ...
    setIdCardNameKh(student.nameKh || "សុខ ចាន់ដារ៉ា");
    setIdCardNameEn(student.nameEn || "SOK CHANDARA");
    setIdCardIdNumber(student.studentId || `STU-26-${String(student.id || "1").padStart(3, '0')}`);
    setIdCardGender(student.gender || "Male");
    setIdCardField1(student.course || "Computer Repair");
    setIdCardField2(student.level || "Level 1");
    setIdCardField3(student.shift || "ថ្ងៃចន្ទ - ថ្ងៃសុក្រ");
    setIdCardField4(student.hours || student.time || "08:00 - 09:00 AM");
    setIdCardPhone(student.phoneNumber || student.guardianPhone || "+855 87 850 014");
    setIdCardAddress(student.pob || (student as any).currentAddress || "ក្បាលស្ពាន២, អូរជ្រៅ, ប៉ោយប៉ែត, បន្ទាយមានជ័យ");
    setIdCardDob(student.dob || "10/05/2005");
    setIdCardIssueDate(student.startDate || "01.01.2026");
    setIdCardExpireDate(student.endDate || "01.01.2027");
    if (student.photoUrl || (student as any).photo || (student as any).studentPhoto || (student as any).avatarUrl) {
      setIdCardPhoto(student.photoUrl || (student as any).photo || (student as any).studentPhoto || (student as any).avatarUrl);
    }
  };
  ...
  const handlePrefillTeacher = (teacher: any) => {
    ...
    setIdCardNameKh(teacher.nameKh || "កែវ សុផល");
    setIdCardNameEn(teacher.nameEn || "KEO SOPHAL");
    setIdCardIdNumber(teacher.teacherId || `TCH-26-${String(teacher.id || "1").padStart(3, '0')}`);
    setIdCardGender(teacher.gender || "Male");
    setIdCardField1(teacher.specialty || "Computer Teaching");

    const spec = (teacher.specialty || teacher.role || teacher.position || "").toLowerCase();
    let defaultRole = "TEACHER";
    if (spec.includes("secr") || spec.includes("លេខា")) {
      defaultRole = "SECRETARY";
    } else if (spec.includes("account") || spec.includes("គណនេយ្យ") || spec.includes("បេឡា")) {
      defaultRole = "ACCOUNTANT";
    } else if (spec.includes("admin") || spec.includes("អភិបាល")) {
      defaultRole = "ADMIN";
    } else if (teacher.role) {
      defaultRole = String(teacher.role).toUpperCase();
    }

    setIdCardField2(teacher.position || teacher.level || "");
    setIdCardField3(defaultRole);
    setIdCardField4(teacher.status || "ACTIVE");
    setIdCardPhone(teacher.phone || "+855 87 850 014");
    setIdCardAddress(teacher.pob || "ក្បាលស្ពាន២, អូរជ្រៅ, ប៉ោយប៉ែត, បន្ទាយមានជ័យ");
    setIdCardDob("15/08/1995");
    if (teacher.photoUrl || teacher.photo) {
      setIdCardPhoto(teacher.photoUrl || teacher.photo);
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, stateSetter: (val: string) => void) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      showToast("ទំហំរូបភាពធំជាង ៥MB មិនអាចអនុញ្ញាតបានទេ!", "error");
      return;
    }
    ...
    try {
      ...
      stateSetter(finalUrl);

      // Auto-save specific system setting images immediately
      if (stateSetter === setSchoolLogo) {
        try { localStorage.setItem("plc_school_logo", finalUrl); } catch(e){}
        fetch("/api/system/settings", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
          body: JSON.stringify({ schoolLogo: finalUrl })
        }).catch(e => console.warn(e));
      } else if (stateSetter === setDeveloperLogo) {
        try { localStorage.setItem("plc_developer_logo", finalUrl); } catch(e){}
        fetch("/api/system/settings", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
          body: JSON.stringify({ developerLogo: finalUrl })
        }).catch(e => console.warn(e));
      } else if (stateSetter === setKhqrImage) {
        try { localStorage.setItem("plc_khqr_image", finalUrl); } catch(e){}
        fetch("/api/system/settings", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
          body: JSON.stringify({ khqrImage: finalUrl })
        }).catch(e => console.warn(e));
      } else if (stateSetter === setIdCardBackgroundFront) {
        try { localStorage.setItem("plc_id_card_bg_front", finalUrl); } catch(e){}
        fetch("/api/system/settings", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
          body: JSON.stringify({ idCardBackgroundFront: finalUrl })
        }).catch(e => console.warn(e));
      } else if (stateSetter === setIdCardBackgroundBack) {
        try { localStorage.setItem("plc_id_card_bg_back", finalUrl); } catch(e){}
        fetch("/api/system/settings", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
          body: JSON.stringify({ idCardBackgroundBack: finalUrl })
        }).catch(e => console.warn(e));
      } else if (stateSetter === setCoverImage) {
        try { localStorage.setItem("plc_cover_image", finalUrl); } catch(e){}
        fetch("/api/system/settings", {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
          body: JSON.stringify({ coverImage: finalUrl, bannerImage: finalUrl })
        }).catch(e => console.warn(e));
      } else if (stateSetter === setIdCardPhoto) {
        if (selectedIdCardStudent && selectedIdCardStudent.id) {
          const updatedStd = { ...selectedIdCardStudent, photoUrl: finalUrl, photo: finalUrl };
          setSelectedIdCardStudent(updatedStd);
          setStudents((prev: any[]) => prev.map(s => s.id === selectedIdCardStudent.id ? updatedStd : s));
          fetch(`/api/students/${selectedIdCardStudent.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
            body: JSON.stringify({ photoUrl: finalUrl })
          }).catch(e => console.warn("Failed to persist student photo:", e));
        } else if (selectedIdCardTeacher && selectedIdCardTeacher.id) {
          const updatedTch = { ...selectedIdCardTeacher, photoUrl: finalUrl, photo: finalUrl };
          setSelectedIdCardTeacher(updatedTch);
          setTeachers((prev: any[]) => prev.map(t => t.id === selectedIdCardTeacher.id ? updatedTch : t));
          fetch(`/api/teachers/${selectedIdCardTeacher.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
            body: JSON.stringify({ photoUrl: finalUrl })
          }).catch(e => console.warn("Failed to persist teacher photo:", e));
        }
      }

      showToast("បញ្ចូល និងរក្សាទុកដោតស្វ័យប្រវត្តជោគជ័យ!", "success");
    } catch (err: any) {
      ...
      >
        ...
        <div className="text-left mt-9 z-10 px-6.5 flex flex-col items-start w-full">
          ...
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[8.5px] font-black uppercase tracking-wider bg-primary-50 border border-primary-100 text-primary-700 mt-2.5">
            {idCardRole === 'student' ? (idCardField1 || 'Microsoft Office Word') : (idCardField1 || 'Teacher | Specialty')}
          </span>
        </div>
        ...
        {/* Barcode Footer */}
        <div className="mt-auto pb-1.5 pt-0.5 flex flex-col items-center justify-center z-10 w-full bg-white overflow-hidden">
          <Barcode
            value={idCardIdNumber || "STU-26-001"}
            format="CODE128"
            width={1.2}
            height={18}
            displayValue={false}
            ...
            lineColor="#000000"
          />
          <span className="text-[7px] font-mono font-bold text-slate-600 mt-0.5 tracking-[0.2em]">{idCardIdNumber || "STU-26-001"}</span>
        </div>
      </div>
    );
  };
        ...
        <div className="px-4.5 py-2.5 flex-1 flex flex-col justify-start z-10">
          ...
          <div className="flex justify-center mt-1 z-10">
            <div className="text-[8px] font-black text-[#0ea5e9] bg-transparent px-3.5 py-0.5 flex items-center gap-1.5">
              ...
              <span className="font-sans font-bold text-slate-700">{idCardAddress || "ក្បាលស្ពាន២, អូរជ្រៅ, ប៉ោយប៉ែត, បន្ទាយមានជ័យ"}</span>
            </div>
          </div>
        ...
        <div className="relative h-[125px] shrink-0 mt-auto z-10 bg-transparent">
          <div className="absolute inset-x-0 bottom-4 flex items-center justify-between px-6 z-10 text-slate-800">
            <div className="bg-white p-1 rounded-sm border border-slate-200/90 shadow-2xs shrink-0 flex items-center justify-center">
              <QRCodeCanvas
                value={`${window.location.origin.includes("ais-dev-") ? window.location.origin.replace("ais-dev-", "ais-pre-") : window.location.origin}/?portal_student=${idCardIdNumber || "STU-26-001"}`}
                size={180}
                level="H"
                fgColor="#000000"
                bgColor="#ffffff"
  ...
  const downloadIdCard = async () => {
    ...
    try {
      ...
      const { safeToPng } = await import('../lib/safe-html-to-image');

      // Helper to temporarily force clean full-opacity styles for download
      const captureCleanImage = async (element: HTMLElement) => {
        ...
        try {
          return await safeToPng(element, {
            pixelRatio: 3,
      ...
      if (idCardPrintSide === 'front' || idCardPrintSide === 'both') {
        const dataUrlFront = await captureCleanImage(frontCardRef.current);
      ...
      if (idCardPrintSide === 'back' || idCardPrintSide === 'both') {
        const dataUrlBack = await captureCleanImage(backCardRef.current);
  ...
  // Fetch initial data sequentially to prevent proxy 429 rate limit
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        // 1. Load settings first as they might be needed quickly
        try {
          const res = await fetch("/api/system/settings", {
            headers: { "Authorization": `Bearer ${token}` }
          });
          ...
          if (res.ok) {
            ...
            if (data) {
              ...
              if (data.schoolLogo) {
                ...
                try { localStorage.setItem("plc_school_logo", data.schoolLogo); } catch(e){}
              } else {
                const storedLogo = localStorage.getItem("plc_school_logo");
                if (storedLogo) {
                  setSchoolLogo(storedLogo);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ schoolLogo: storedLogo })
                  }).catch(e => console.warn(e));
                }
              }

              if (data.schoolStamp) {
                setSchoolStamp(data.schoolStamp);
                try { localStorage.setItem("plc_school_stamp", data.schoolStamp); } catch(e){}
              } else {
                const storedStamp = localStorage.getItem("plc_school_stamp");
                if (storedStamp) {
                  setSchoolStamp(storedStamp);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ schoolStamp: storedStamp })
                  }).catch(e => console.warn(e));
                }
              }
              ...
              if (data.directorSignature) {
                ...
                try { localStorage.setItem("plc_director_signature", data.directorSignature); } catch(e){}
              } else {
                const storedSig = localStorage.getItem("plc_director_signature");
                if (storedSig) {
                  setDirectorSignature(storedSig);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ directorSignature: storedSig })
                  }).catch(e => console.warn(e));
                }
              }

              if (data.coverImage || data.bannerImage) {
                const img = data.coverImage || data.bannerImage;
                setCoverImage(img);
                try { localStorage.setItem("plc_cover_image", img); } catch(e){}
              } else {
                const storedCover = localStorage.getItem("plc_cover_image");
                if (storedCover) {
                  setCoverImage(storedCover);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ coverImage: storedCover, bannerImage: storedCover })
                  }).catch(e => console.warn(e));
                }
              }
              ...
              if (data.khqrImage) {
                setKhqrImage(data.khqrImage);
                try { localStorage.setItem("plc_khqr_image", data.khqrImage); } catch(e){}
              } else {
                const storedKhqr = localStorage.getItem("plc_khqr_image");
                if (storedKhqr) {
                  setKhqrImage(storedKhqr);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ khqrImage: storedKhqr })
                  }).catch(e => console.warn(e));
                }
              }

              if (data.developerLogo) {
                setDeveloperLogo(data.developerLogo);
                try { localStorage.setItem("plc_developer_logo", data.developerLogo); } catch(e){}
              } else {
                const storedDevLogo = localStorage.getItem("plc_developer_logo");
                if (storedDevLogo) {
                  setDeveloperLogo(storedDevLogo);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ developerLogo: storedDevLogo })
                  }).catch(e => console.warn(e));
                }
              }

              if (data.idCardBackgroundFront) {
                setIdCardBackgroundFront(data.idCardBackgroundFront);
                try { localStorage.setItem("plc_id_card_bg_front", data.idCardBackgroundFront); } catch(e){}
              } else {
                const storedFrontBg = localStorage.getItem("plc_id_card_bg_front");
                if (storedFrontBg) {
                  setIdCardBackgroundFront(storedFrontBg);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ idCardBackgroundFront: storedFrontBg })
                  }).catch(e => console.warn(e));
                }
              }

              if (data.idCardBackgroundBack) {
                setIdCardBackgroundBack(data.idCardBackgroundBack);
                try { localStorage.setItem("plc_id_card_bg_back", data.idCardBackgroundBack); } catch(e){}
              } else {
                const storedBackBg = localStorage.getItem("plc_id_card_bg_back");
                if (storedBackBg) {
                  setIdCardBackgroundBack(storedBackBg);
                  fetch("/api/system/settings", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                    body: JSON.stringify({ idCardBackgroundBack: storedBackBg })
                  }).catch(e => console.warn(e));
                }
              }
              ...
              if (data.courseOptions && Array.isArray(data.courseOptions)) {
                ...
                setCourseOptions(data.courseOptions.filter((co: string) => !duplicatesToRemove.includes(co)));
              }
          ...
          console.warn("Notice: Settings fetch skipped or unavailable:", err);
        }
        ...
        try {
          ...
          if (res.ok) {
            ...
            console.error("Failed to load teachers:", res.status, await res.text());
          }
        } catch (err) {
          console.error("Error fetching teachers:", err);
        }

        // 4. Load timetables & courses
        fetchTimetables();
        fetchDbCourses();

        // 5. Load financial transactions & salaries if authorized
        if (user?.role === "ADMIN" || user?.role === "ACCOUNTANT") {
          ...
          try {
            ...
            if (res.ok) {
              const data = await safeJson(res);
              if (data && data.salaries) setSalaries(data.salaries);
            }
          } catch (err) {
            console.warn("Error fetching salaries:", err);
          }
        }
      } catch (e) {
  ...
  const openEditStudentModal = (student: StudentType) => {
    ...
    setRegPhotoUrl((student as any).photoUrl || "");
  ...
  const handleConfirmDeleteTeacher = () => {
    ...
    fetch(`/api/teachers/${id}`, {
      ...
      .then(res => {
        if (res.ok) {
          setTeachers(prev => prev.filter(t => t && t.id !== id));
  ...
  const activeCourseList = useMemo(() => {
    return Array.from(new Set(students.map(s => s && s.course).filter(Boolean))) as string[];
  }, [students]);
  ...
  const enrollmentData = useMemo(() => {
    return finalCourseList.map(courseName => {
      return {
        ...
        fullName: courseName,
        [femaleKey]: students.filter(s => s && s.course === courseName && s.gender === "Female").length,
        [maleKey]: students.filter(s => s && s.course === courseName && s.gender === "Male").length
      };
    });
  ...
  const chartData: any[] = useMemo(() => {
    return chartTab === 'courses' ? enrollmentData : finalCourseList.map(courseName => {
      return {
        name: translateCourseOrSpecialtyName(courseName, uiLang),
        fullName: courseName,
        [collectedKey]: students.filter(s => s && s.course === courseName).reduce((sum, s) => sum + s.paid, 0),
        [dueKey]: students.filter(s => s && s.course === courseName).reduce((sum, s) => sum + s.due, 0)
      };
    });
  ...
  const teachersData = useMemo(() => {
    const maleTeachers = teachers.filter(t => t && t.gender === 'Male').length;
    const femaleTeachers = teachers.filter(t => t && t.gender === 'Female').length;
    return [
  ...
  const studentMetrics = useMemo(() => {
    const completedList = students.filter(s => s && s.status === 'COMPLETED');
    const completedTotal = completedList.length;
    const completedFemale = completedList.filter(s => s && s.gender === 'Female').length;
    const completedMale = completedList.filter(s => s && s.gender === 'Male').length;
    ...
    const stoppedList = students.filter(s => s && s.status === 'STOP');
    const stoppedTotal = stoppedList.length;
    const stoppedFemale = stoppedList.filter(s => s && s.gender === 'Female').length;
    const stoppedMale = stoppedList.filter(s => s && s.gender === 'Male').length;
    ...
    const studyingList = students.filter(s => s && s.status === 'STUDYING');
    const studyingTotal = studyingList.length;
    const studyingFemale = studyingList.filter(s => s && s.gender === 'Female').length;
    const studyingMale = studyingList.filter(s => s && s.gender === 'Male').length;
    ...
    const femaleList = students.filter(s => s && s.gender === 'Female');
    const femaleCount = femaleList.length;
    const femaleStudying = femaleList.filter(s => s && s.status === 'STUDYING').length;
    const femaleStopped = femaleList.filter(s => s && s.status === 'STOP').length;
    const femaleCompleted = femaleList.filter(s => s && s.status === 'COMPLETED').length;

    const maleList = students.filter(s => s && s.gender === 'Male');
    const maleCount = maleList.length;
    const maleStudying = maleList.filter(s => s && s.status === 'STUDYING').length;
    const maleStopped = maleList.filter(s => s && s.status === 'STOP').length;
    const maleCompleted = maleList.filter(s => s && s.status === 'COMPLETED').length;
  ...
  const activeCourseMetrics = useMemo(() => {
    const filtered = activeCourseFilter
      ? students.filter(s => s && s.course === activeCourseFilter)
      : students;

    const totalCount = filtered.length;
    const completedCount = filtered.filter(s => s && s.status === "COMPLETED").length;
    const stopCount = filtered.filter(s => s && s.status === "STOP").length;
    const studyingCount = filtered.filter(s => s && s.status === "STUDYING").length;
    ...
    const uniqueCourses = Array.from(new Set(students.map(s => s && s.course).filter(Boolean))) as string[];
    const uniqueCoursesCount = uniqueCourses.length;

    const femaleStudents = filtered.filter(s => s && s.gender === "Female");
    const maleStudents = filtered.filter(s => s && s.gender === "Male");
    ...
    const femaleStudying = femaleStudents.filter(s => s && s.status === "STUDYING").length;
    const femaleStopped = femaleStudents.filter(s => s && s.status === "STOP").length;
    const femaleCompleted = femaleStudents.filter(s => s && s.status === "COMPLETED").length;

    const maleStudying = maleStudents.filter(s => s && s.status === "STUDYING").length;
    const maleStopped = maleStudents.filter(s => s && s.status === "STOP").length;
    const maleCompleted = maleStudents.filter(s => s && s.status === "COMPLETED").length;
  ...
  const menuGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "Course Final Exam", labelKh: "ប្រឡងបញ្ចប់វគ្គសិក្សា", labelEn: "Course Final Exam", icon: GraduationCap },
        { id: "Pre-Course Test", labelKh: "ប្រឡងតេស្ដ មុនវគ្គសិក្សា", labelEn: "Pre-Course Test", icon: Compass },
  ...
  return (
    <div className={`h-full w-full app-background text-slate-800 font-sans flex relative select-none overflow-hidden ${isFullscreen ? "fixed inset-0 z-[99999] w-screen h-screen bg-slate-900" : ""}`}>
      {/* Toast Notifications */}
      <AnimatePresence>
        {toasts.map((toast) => (
          <motion.div
      ...
      <aside
        ...
        className={`fixed inset-y-0 left-0 z-40 w-56 text-slate-800 flex flex-col justify-between border-r border-slate-200/90 transition-all duration-300 transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:h-screen lg:shrink-0`}
      >
              ...
              return (
                <div key={groupIndex}>
                  <button 
                    ...
                    className="w-full px-2 mb-1.5 flex items-center justify-between transition-colors cursor-pointer text-left group"
                    style={{ color: sidebarTextColor }}
                  >
                    <span className="text-[10.5px] font-black tracking-widest uppercase font-sans" style={{ opacity: 0.6 }}>
                      {uiLang === "kh" ? group.titleKh : 
                       uiLang === "en" ? group.titleEn : group.titleZh}
                    </span>
                    <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`} style={{ opacity: 0.6 }} />
                  </button>
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div 
                      ...
                      >
                        {group.items.map((item) => {
                          ...
                          const activeClasses = isSelected
                            ? "font-extrabold shadow-3xs rounded-r-xl rounded-l-xs border-l-[3.5px]"
                            : "hover:bg-slate-100/90 hover:translate-x-0.5 rounded-r-xl rounded-l-xs border-l-[3.5px] border-blue-300/80 hover:border-blue-600 bg-slate-50/30";
                          ...
                          return (
                            <button
                              ...
                              style={isSelected ? { backgroundColor: sidebarActiveColor, borderColor: sidebarActiveColor, color: '#ffffff' } : { color: sidebarTextColor }}
                            ...
                            >
                              <IconComp className={`w-4 h-4 shrink-0 transition-transform ${isSelected ? "opacity-100 scale-105" : "opacity-70"}`} />
                              <div className="flex flex-col items-start leading-none gap-0.5 text-left">
                                <span className={`text-[12px] tracking-wide ${isSelected ? "font-black" : "font-bold"}`}>
                                ...
                                <span className={`text-[9px] font-semibold normal-case tracking-normal`} style={{ opacity: isSelected ? 0.9 : 0.6 }}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header
          ...
          className="sticky top-0 z-30 text-white border-b border-white/20 h-14 sm:h-16 shrink-0 transition-colors duration-200"
        >
          <div className="h-full px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-3 relative">
                          ...
                          <>
                            ...
                            {globalSearchResults.students.length > 0 && (
                              <div>
                                ...
                                <div className="space-y-1">
                                  {globalSearchResults.students.map((st: any) => (
                                    <button
                                      key={st.id}
                                      type="button"
                                      onClick={() => {
                                        setActiveTab("Students");
                                        setStudentSearch(st.name || st.nameKh || st.id);
                                        setSelectedStudent(st);
                                        setIsViewStudentModalOpen(true);
                                        addRecentSearch(st.name || st.nameKh || st.id);
                                        setGlobalSearchQuery("");
                                        setIsSearchFocused(false);
                                        showToast(`បានបើកព័ត៌មានសិស្ស៖ ${st.name || st.nameKh || st.id}`, 'info');
                                      }}
                                      ...
                                      <div className="flex items-center gap-2.5 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                          ...
                                          <div className="text-[10px] text-slate-400 truncate flex items-center gap-2">
                                            ...
                                            {(st.phone || st.phoneNumber || st.guardianPhone) && <span>• {st.phone || st.phoneNumber || st.guardianPhone}</span>}
                                          </div>
                                        </div>
                                      </div>
            ...
            {/* Right side: Language Selector, Theme Palette Selector, User card & Log out button */}
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                              ...
                              <div>
                                ...
                                <div className="space-y-1">
                                  {globalSearchResults.students.map((st: any) => (
                                    ...
                                    >
                                      <div>
                                        <div className="font-extrabold text-slate-800 text-[12px]">{st.name || st.nameKh} ({st.id})</div>
                                        <div className="text-[10px] text-slate-400">{st.course || st.phone || st.phoneNumber}</div>
                                      </div>
                                      <ChevronRight className="w-3.5 h-3.5 text-slate-300" />
                                    </button>
                                  ))}
              ...
              <div className="relative shrink-0" ref={themeMenuRef}>
                <button
                  ...
                  title={uiLang === "kh" ? "ផ្លាស់ប្តូរពណ៌ប្រធានបទ (Change Theme Color)" : "Change Theme Color"}
                >
                ...
                <AnimatePresence>
                  {isThemeMenuOpen && (
                    <motion.div
                      ...
                      transition={{ duration: 0.15 }}
                      className="absolute right-0 mt-2 w-72 sm:w-80 max-h-[85vh] overflow-y-auto bg-white/95 backdrop-blur-2xl rounded-2xl border border-slate-200/90 shadow-2xl z-50 p-4 font-sans text-left"
                    >
                      <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-3">
                        <h4 className="text-xs font-black text-slate-900 leading-none flex items-center gap-1.5">
                          <Palette className="w-3.5 h-3.5 text-blue-600" />
                          {uiLang === "kh" ? "ឧបករណ៍កំណត់ពណ៌ (Color Tools)" : "Color Tools"}
                        </h4>
                        <button
                          type="button"
                      ...
                      <div className="space-y-4 text-xs">
                        {/* Header Bar Color Tool */}
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-700">
                              {uiLang === "kh" ? "១. ពណ៌របាខាងលើ (Header Bar)" : "1. Top Header Bar Color"}
                            </span>
                            <div className="flex items-center gap-1.5">
                              <label className="relative w-6 h-6 rounded-full border border-slate-300 shadow-3xs cursor-pointer flex items-center justify-center overflow-hidden transition-transform hover:scale-110" style={{ backgroundColor: headerBgColor }}>
                                <input
                                  type="color"
                                  value={headerBgColor.startsWith('#') ? headerBgColor : '#3b82f6'}
                                  onChange={(e) => setHeaderBgColor(e.target.value)}
                                  className="opacity-0 absolute inset-0 w-full h-full cursor-pointer"
                                />
                              </label>
                              <span className="text-[10px] font-mono text-slate-500 uppercase">{headerBgColor}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                            {['#3b82f6', '#2563eb', '#1d4ed8', '#0284c7', '#0f766e', '#15803d', '#b91c1c', '#6b21a8', '#1e293b', '#0f172a'].map((color) => (
                              <button
                                key={color}
                                type="button"
                                onClick={() => setHeaderBgColor(color)}
                                className={`w-6 h-6 rounded-full border transition-all cursor-pointer ${headerBgColor === color ? 'ring-2 ring-blue-500 ring-offset-1 scale-110' : 'hover:scale-105 border-slate-200'}`}
                        ...
                        {/* Menu/Sidebar Panel Color Tool */}
                        <div className="space-y-1.5 pt-2 border-t border-slate-100">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-700">
                              {uiLang === "kh" ? "២. ពណ៌ផ្ទាំង MENU (Sidebar Bg)" : "2. Menu Panel Background"}
                            </span>
                            <div className="flex items-center gap-1.5">
                              <label className="relative w-6 h-6 rounded-full border border-slate-300 shadow-3xs cursor-pointer flex items-center justify-center overflow-hidden transition-transform hover:scale-110" style={{ backgroundColor: sidebarBgColor }}>
                                <input
                                  type="color"
                                  value={sidebarBgColor.startsWith('#') ? sidebarBgColor : '#ffffff'}
                                  onChange={(e) => setSidebarBgColor(e.target.value)}
                                  className="opacity-0 absolute inset-0 w-full h-full cursor-pointer"
                                />
                              </label>
                              <span className="text-[10px] font-mono text-slate-500 uppercase">{sidebarBgColor}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                            {['#ffffff', '#f8fafc', '#f1f5f9', '#e2e8f0', '#eff6ff', '#f0fdf4', '#fef2f2', '#1e293b', '#0f172a', '#111827'].map((color) => (
                              <button
                                key={color}
                                type="button"
                                onClick={() => setSidebarBgColor(color)}
                                className={`w-6 h-6 rounded-full border transition-all cursor-pointer ${sidebarBgColor.toLowerCase() === color.toLowerCase() ? 'ring-2 ring-blue-500 ring-offset-1 scale-110' : 'hover:scale-105 border-slate-200'}`}
                                style={{ backgroundColor: color }}
                                title={color}
                              />
                            ))}
                        ...
                        {/* Menu Text Color Tool */}
                        <div className="space-y-1.5 pt-2 border-t border-slate-100">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-700">
                              {uiLang === "kh" ? "៣. ពណ៌អក្សរលើផ្ទាំង MENU (Menu Text)" : "3. Menu Text Color"}
                            </span>
                            <div className="flex items-center gap-1.5">
                              <label className="relative w-6 h-6 rounded-full border border-slate-300 shadow-3xs cursor-pointer flex items-center justify-center overflow-hidden transition-transform hover:scale-110" style={{ backgroundColor: sidebarTextColor }}>
                                <input
                                  type="color"
                                  value={sidebarTextColor.startsWith('#') ? sidebarTextColor : '#334155'}
                                  onChange={(e) => setSidebarTextColor(e.target.value)}
                                  className="opacity-0 absolute inset-0 w-full h-full cursor-pointer"
                                />
                              </label>
                              <span className="text-[10px] font-mono text-slate-500 uppercase">{sidebarTextColor}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                            {['#334155', '#1e293b', '#0f172a', '#2563eb', '#16a34a', '#dc2626', '#9333ea', '#ffffff'].map((color) => (
                              <button
                                key={color}
                                type="button"
                                onClick={() => setSidebarTextColor(color)}
                                className={`w-6 h-6 rounded-full border transition-all cursor-pointer ${sidebarTextColor.toLowerCase() === color.toLowerCase() ? 'ring-2 ring-blue-500 ring-offset-1 scale-110' : 'hover:scale-105 border-slate-200'}`}
                                style={{ backgroundColor: color }}
                                title={color}
                              />
                            ))}
                        ...
                        {/* Selected Button Color Tool */}
                        <div className="space-y-1.5 pt-2 border-t border-slate-100">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-700">
                              {uiLang === "kh" ? "៤. ពណ៌ប៊ូតុងពេលសេលិច (Selected Button)" : "4. Selected Button Color"}
                            </span>
                            <div className="flex items-center gap-1.5">
                              <label className="relative w-6 h-6 rounded-full border border-slate-300 shadow-3xs cursor-pointer flex items-center justify-center overflow-hidden transition-transform hover:scale-110" style={{ backgroundColor: sidebarActiveColor }}>
                                <input
                                  type="color"
                                  value={sidebarActiveColor.startsWith('#') ? sidebarActiveColor : '#3b82f6'}
                                  onChange={(e) => setSidebarActiveColor(e.target.value)}
                                  className="opacity-0 absolute inset-0 w-full h-full cursor-pointer"
                                />
                              </label>
                              <span className="text-[10px] font-mono text-slate-500 uppercase">{sidebarActiveColor}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                            {['#3b82f6', '#2563eb', '#1d4ed8', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#0f172a'].map((color) => (
                              <button
                                key={color}
                                type="button"
                                onClick={() => setSidebarActiveColor(color)}
                                className={`w-6 h-6 rounded-full border transition-all cursor-pointer ${sidebarActiveColor.toLowerCase() === color.toLowerCase() ? 'ring-2 ring-blue-500 ring-offset-1 scale-110' : 'hover:scale-105 border-slate-200'}`}
                                style={{ backgroundColor: color }}
                                title={color}
                              />
                            ))}
                        ...
                        {/* App Background Color Tool */}
                        <div className="space-y-1.5 pt-2 border-t border-slate-100">
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-slate-700">
                              {uiLang === "kh" ? "៥. ពណ៌ផ្ទៃខាងក្រោយ (Background)" : "5. Background Color"}
                            </span>
                            <div className="flex items-center gap-1.5">
                              <label className="relative w-6 h-6 rounded-full border border-slate-300 shadow-3xs cursor-pointer flex items-center justify-center overflow-hidden transition-transform hover:scale-110" style={{ backgroundColor: appBgColor || '#f8fafc' }}>
                                <input
                                  type="color"
                              ...
                              <span className="text-[10px] font-mono text-slate-500 uppercase">{appBgColor || '#F8FAFC'}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                            {['#f8fafc', '#f1f5f9', '#e2e8f0', '#eff6ff', '#f0fdf4', '#fef2f2', '#fff7ed', '#fefce8', '#1e293b', '#0f172a'].map((color) => (
                              <button
                                key={color}
                                type="button"
                                onClick={() => setAppBgColor(color)}
                                className={`w-6 h-6 rounded-full border transition-all cursor-pointer ${appBgColor === color ? 'ring-2 ring-blue-500 ring-offset-1 scale-110' : 'hover:scale-105 border-slate-200'}`}
                                style={{ backgroundColor: color }}
                                title={color}
                              />
                            ))}
                        ...
                        {(() => {
                          ...
                          const appThemeHex = appTheme.startsWith('#') ? appTheme : (THEME_PRESETS[appTheme] || '#3b82f6');
                          return (
                            <div className="space-y-1.5 pt-2 border-t border-slate-100">
                              <div className="flex items-center justify-between">
                                <span className="font-bold text-slate-700">
                                  {uiLang === "kh" ? "៦. ពណ៌ចម្បង (Theme Accent)" : "6. Theme Accent Color"}
                                </span>
                                <div className="flex items-center gap-1.5">
                                  <label className="relative w-6 h-6 rounded-full border border-slate-300 shadow-3xs cursor-pointer flex items-center justify-center overflow-hidden transition-transform hover:scale-110" style={{ backgroundColor: appThemeHex }}>
                                    <input
                                      type="color"
                                      value={appThemeHex ?? ""}
                                  ...
                                  <span className="text-[10px] font-mono text-slate-500 uppercase">{appThemeHex}</span>
                                </div>
                              </div>
                              <div className="flex items-center gap-1.5 flex-wrap pt-0.5">
                                {['#3b82f6', '#6366f1', '#8b5cf6', '#ec4899', '#f43f5e', '#10b981', '#14b8a6', '#0ea5e9', '#f59e0b', '#1e293b'].map((color) => (
                                  <button
                                    key={color}
                                    type="button"
                                    onClick={() => setAppTheme(color)}
                                    className={`w-6 h-6 rounded-full border transition-all cursor-pointer ${appThemeHex.toLowerCase() === color.toLowerCase() ? 'ring-2 ring-blue-500 ring-offset-1 scale-110' : 'hover:scale-105 border-slate-200'}`}
                                    style={{ backgroundColor: color }}
                                    title={color}
                                  />
                                ))}
                        ...
                        <div className="pt-2 border-t border-slate-100 flex justify-end">
                          <button
                            ...
                            onClick={() => {
                              ...
                              setSidebarTextColor('#334155');
                              setSidebarActiveColor('#3b82f6');
                              setAppBgColor('#f8fafc');
                              setAppTheme('indigo');
                            }}
                            className="text-[11px] font-semibold text-slate-500 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
                          >
                            {uiLang === "kh" ? "កំណត់ឡើងវិញ (Reset)" : "Reset Default"}
                          </button>
                        </div>
                      </div>
                    </motion.div>
        ...
        >
          {activeTab === "Dashboard" && (
            <DashboardTab 
              ...
              setDeveloperLogo={setDeveloperLogo} 
              developerName={developerName} 
              developerKhmerName={developerKhmerName} 
              developerPhone={developerPhone} 
              developerTelegram={developerTelegram} 
              students={students} 
              activeCourseFilter={activeCourseFilter} 
              chartTab={chartTab} 
              activeCourseMetrics={activeCourseMetrics} 
              ...
              femaleKey={femaleKey} 
              chartData={chartData} 
              toKhmerNumeral={toKhmerNumeral} 
              getCourseSubtitle={getCourseSubtitle} 
              ...
              setSidebarActiveColor={setSidebarActiveColor}
              appTheme={appTheme}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[160px] overflow-y-auto pr-1">
                            {displaySchedules.map((sched, idx) => {
                              // Find teacher
                              const teacherObj = teachers.find(t => t && (t.id === sched.teacherId || t.teacherId === sched.teacherId));
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-slate-200/80 lg:hidden shadow-[0_-4px_25px_rgba(0,0,0,0.06)] pb-[max(0.4rem,env(safe-area-inset-bottom))] pt-1">
        <div className="flex justify-around items-center h-13 w-full max-w-lg mx-auto px-1 sm:px-3">
          {(() => {
            const filteredMenu = menuItems.filter(i => {
              ...
              if (user?.role === "TEACHER") return ["Dashboard", "Students", "Attendance"].includes(i.id);
            ...
            const getShortLabel = (id: string, labelKh: string, labelEn: string) => {
              if (uiLang === "kh") {
                ...
                if (id === "Teachers") return "គ្រូ";
                if (id === "Courses") return "វគ្គសិក្សា";
                if (id === "Attendance") return "វត្តមាន";
                if (id === "Analytics") return "របាយការណ៍";
                if (id === "Announcements") return "ផ្សព្វផ្សាយ";
                if (id === "Finance") return "ហិរញ្ញវត្ថុ";
                if (id === "Timetable") return "កាលវិភាគ";
                if (id === "Grading") return "ពិន្ទុ";
                return labelKh;
              } else if (uiLang === "en") {
                ...
                if (id === "Analytics") return "Reports";
                if (id === "Announcements") return "Promote";
                if (id === "Finance") return "Finance";
                if (id === "Timetable") return "Timetable";
                if (id === "Grading") return "Grades";
                return labelEn;
              } else {
                ...
                if (id === "Analytics") return "报表";
                if (id === "Announcements") return "公告";
                return labelEn;
              }
            };
            ...
            return (
              <>
                {leftItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 min-w-0 flex flex-col items-center justify-center transition-all duration-150 cursor-pointer relative py-0.5 focus:outline-none select-none ${
                        isSelected 
                          ? "font-bold" 
                          : "font-medium hover:opacity-80"
                      }`}
                      style={isSelected ? { color: sidebarActiveColor } : { color: sidebarTextColor, opacity: 0.7 }}
                    >
                      <IconComp className={`w-5 h-5 ${isSelected ? "stroke-[2.2] scale-105" : "stroke-[1.8]"}`} />
                      <span className="text-[10px] leading-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-full px-0.5 mt-0.5">
                ...
                <button
                  ...
                  className="flex flex-col items-center justify-center w-11 h-11 rounded-full bg-gradient-to-tr from-primary-600 to-blue-500 text-white shadow-lg shadow-primary-500/30 ring-4 ring-white active:scale-95 transition-all duration-200 cursor-pointer shrink-0 -mt-4 mx-1.5 focus:outline-none select-none relative z-10"
                ...
                {(() => {
                  ...
                  return (
                    <button
                      ...
                      style={isMoreSelected ? { color: sidebarActiveColor } : { color: sidebarTextColor, opacity: 0.7 }}
                    >
                      <Menu className={`w-5 h-5 ${isMoreSelected ? "stroke-[2.2] scale-105" : "stroke-[1.8]"}`} />
                      <span className="text-[10px] leading-tight whitespace-nowrap overflow-hidden text-ellipsis px-0.5 mt-0.5">
