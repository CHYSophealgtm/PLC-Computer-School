...
export default function CoursesTab({ uiLang, token: propToken, students: propStudents, setStudents: propSetStudents, syncCourseOption, courseOptions = [] }: { uiLang?: string, token?: string, students?: any[], setStudents?: any, syncCourseOption?: (title: string) => void, courseOptions?: string[] } = {}) {
  ...
  const selectedCourseSchedules = useMemo(() => {
    ...
    return timetables.filter(t => t && t.subject?.trim().toLowerCase() === title.trim().toLowerCase());
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const promises: any[] = [
        safeFetchObj('/api/courses'),
        safeFetchObj('/api/teachers'),
        safeFetchObj('/api/timetables')
      ];
      
      if (!propStudents) {
        promises.push(safeFetchObj('/api/students'));
      }
      ...
      console.warn("Notice: Courses data fetch fallback:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEnrollSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const selectedStudent = students.find(s => s && s.id === enrollData.studentId);
      const selectedCourse = courses.find(c => c && c.id === enrollData.courseId);
  ...
  const selectedTeacher = useMemo(() => {
    return teachers.find(t => t && t.id === formData.teacherId);
  ...
  const selectedStudent = useMemo(() => {
    return students.find(s => s && s.id === enrollData.studentId);
  }, [students, enrollData.studentId]);

  const filteredEnrollCourses = useMemo(() => {
    const searchLower = courseSearch.toLowerCase();
    return courses.filter(c => c && c.title.toLowerCase().includes(searchLower));
  }, [courses, courseSearch]);

  const selectedEnrollCourse = useMemo(() => {
    return courses.find(c => c && c.id === enrollData.courseId);
  ...
  const selectedEditTeacher = useMemo(() => {
    return teachers.find(t => t && t.id === editFormData.teacherId);
  ...
  const enrolledStudents = useMemo(() => {
    ...
    return students.filter((s: any) => s && enrolledIds.includes(s.id));
  ...
  const stats = useMemo(() => {
    ...
    const activeTeachers = Array.from(new Set(courses.filter(c => c && c.teacherId).map(c => c.teacherId))).length;
              ...
              <div className="flex-1">
                <SearchableSelect
                  ...
                  options={[
                    ...
                    ...teachers
                      .filter(t => t && t.status !== 'EXITED' && t.status !== 'STOP' && t.status !== 'RESIGNED')
                      .map(t => ({
                        value: t.id,
                  ...
                  <tr>
                    <td colSpan={7} className="py-20 text-center">
                      <div className="flex flex-col items-center justify-center gap-3 max-w-md mx-auto px-4">
                        ...
                        <p className="text-xs text-slate-400">{localIdt("អ្នកអាចបន្ថែមវគ្គសិក្សាដោយខ្លួនឯង។", "You can add custom courses here.")}</p>
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <div className="flex flex-col gap-1">
                            ...
                            {(() => {
                              const courseSchedules = timetables.filter(t => t && t.subject?.trim().toLowerCase() === course.title?.trim().toLowerCase());
