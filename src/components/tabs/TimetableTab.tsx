...
// Helper utilities for smart Khmer/English Day matching and Range expansion
export const normalizeDayStr = (str: string): string => {
  if (!str) return "";
  let clean = str.trim().toLowerCase();
  clean = clean.replace(/^ថ្ងៃ/g, "").replace(/\s+/g, " ");
  clean = clean.replace(/ចន្ទ/g, "ច័ន្ទ");
  clean = clean.replace(/ព្រហស្បត៍/g, "ព្រហស្បតិ៍");
  return clean;
};

export const getExpandedDays = (dayStr: string): string[] => {
  const norm = normalizeDayStr(dayStr);
  
  if (
    norm.includes("ច័ន្ទ - សុក្រ") || 
    norm.includes("ច័ន្ទ-សុក្រ") || 
    norm.includes("ច័ន្ទ ដល់ សុក្រ") ||
    norm.includes("mon-fri") ||
    norm.includes("monday-friday") ||
    norm.includes("mon - fri")
  ) {
    return ["ច័ន្ទ", "អង្គារ", "ពុធ", "ព្រហស្បតិ៍", "សុក្រ"];
  }

  if (
    norm.includes("សៅរ៍ - អាទិត្យ") || 
    norm.includes("សៅរ៍-អាទិត្យ") || 
    norm.includes("សៅរ៍ ដល់ អាទិត្យ") ||
    norm.includes("sat-sun") ||
    norm.includes("saturday-sunday") ||
    norm.includes("sat - sun")
  ) {
    return ["សៅរ៍", "អាទិត្យ"];
  }

  if (norm.includes("ពុធ") && norm.includes("សុក្រ") && norm.includes("ច័ន្ទ")) {
    return ["ច័ន្ទ", "ពុធ", "សុក្រ"];
  }

  if (norm.includes("អង្គារ") && norm.includes("ព្រហស្បតិ៍") && norm.includes("សៅរ៍")) {
    return ["អង្គារ", "ព្រហស្បតិ៍", "សៅរ៍"];
  }

  if (norm.includes("រៀងរាល់ថ្ងៃ") || norm.includes("ច័ន្ទ - អាទិត្យ") || norm.includes("everyday")) {
    return ["ច័ន្ទ", "អង្គារ", "ពុធ", "ព្រហស្បតិ៍", "សុក្រ", "សៅរ៍", "អាទិត្យ"];
  }

  const singleDays = ["ច័ន្ទ", "អង្គារ", "ពុធ", "ព្រហស្បតិ៍", "សុក្រ", "សៅរ៍", "អាទិត្យ"];
  const matched = singleDays.filter(d => norm.includes(d.toLowerCase()) || norm.includes(normalizeDayStr(d)));
  if (matched.length > 0) {
    return matched;
  }
...
export const areDaysOverlapping = (day1: string, day2: string): boolean => {
  if (!day1 || !day2) return false;
  if (day1.trim().toLowerCase() === day2.trim().toLowerCase()) return true;
  
  const expanded1 = getExpandedDays(day1);
  const expanded2 = getExpandedDays(day2);
  
  return expanded1.some(d1 => expanded2.some(d2 => normalizeDayStr(d1) === normalizeDayStr(d2)));
};

export const isDayMatching = (slotDay: string, filterDay: string): boolean => {
  if (!filterDay) return true;
  if (!slotDay) return false;
  
  if (slotDay.trim().toLowerCase() === filterDay.trim().toLowerCase()) return true;
  
  return areDaysOverlapping(slotDay, filterDay);
};
...
export default function TimetableTab({ 
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const merged = [...parsed];
          ["ច័ន្ទ - សុក្រ", "សៅរ៍ - អាទិត្យ"].forEach(preset => {
            if (!merged.some(item => normalizeDayStr(item) === normalizeDayStr(preset))) {
    ...
    setDayOptions(DEFAULT_DAY_PRESETS);
  }, []);

  const [dbCourses, setDbCourses] = useState<any[]>([]);
  const [isCustomSubject, setIsCustomSubject] = useState(false);

  const [deletedTrigger, setDeletedTrigger] = useState(0);
  ...
  const mergedSubjects = useMemo(() => {
    ...
    // Fallback defaults if nothing in list
    if (list.size === 0) {
      ["Word", "Excel", "Photoshop", "Python", "Web Development", "UI/UX Design"].forEach(item => list.add(item));
    }
    ...
    const result = Array.from(list).filter(item => 
      !deletedList.some(d => d.trim().toLowerCase() === item.trim().toLowerCase())
    );

    return result.sort((a, b) => a.localeCompare(b, 'km'));
  }, [dbCourses, courseOptions, deletedTrigger]);
  ...
  const handleAddCourseByName = (newValue: string) => {
    ...
    try {
      ...
      if (stored) {
        ...
        const updated = list.filter(item => item.trim().toLowerCase() !== trimmed.toLowerCase());
        localStorage.setItem("plc_deleted_courses", JSON.stringify(updated));
      }
    } catch (e) {}

    if (onAddCourseOption) {
      onAddCourseOption(trimmed);
    }
    setDeletedTrigger(prev => prev + 1);
  };

  const handleEditCourseByName = async (oldValue: string, newValue: string) => {
    const trimmedOld = oldValue.trim();
    const trimmedNew = newValue.trim();
    if (!trimmedNew || trimmedOld === trimmedNew) return;

    // Remove new value from blacklist if present
    try {
      const stored = localStorage.getItem("plc_deleted_courses");
      if (stored) {
        const list: string[] = JSON.parse(stored);
        const updated = list.filter(item => item.trim().toLowerCase() !== trimmedNew.toLowerCase());
        localStorage.setItem("plc_deleted_courses", JSON.stringify(updated));
      }
    } catch (e) {}

    // 1. Edit or add in courseOptions settings
    const idx = courseOptions.indexOf(trimmedOld);
    if (idx !== -1 && onEditCourseOption) {
      onEditCourseOption(idx, trimmedNew);
    } else if (onAddCourseOption) {
      onAddCourseOption(trimmedNew);
    }

    // 2. Also update database course if exists in dbCourses
    const matchedDbCourse = dbCourses.find(c => c.title?.trim().toLowerCase() === trimmedOld.toLowerCase());
    if (matchedDbCourse) {
      try {
        await fetch(`/api/courses/${matchedDbCourse.id}`, { 
          method: 'PUT',
          headers: { 
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}` 
          },
          body: JSON.stringify({ title: trimmedNew })
        });
      } catch (error) {
        console.error("Failed to edit database course in TimetableTab:", error);
      }
    }

    // 3. Update active form state if matching
    if (formData.subject === trimmedOld) {
      setFormData(prev => ({ ...prev, subject: trimmedNew }));
    }
  ...
  const handleDeleteCourseByName = async (value: string) => {
    ...
    try {
      ...
      if (!list.some(d => d.trim().toLowerCase() === trimmed.toLowerCase())) {
        list.push(trimmed);
    ...
    if (onDeleteCourseOption) {
      const idx = courseOptions.findIndex(c => c.trim().toLowerCase() === trimmed.toLowerCase());
      if (idx !== -1) {
        onDeleteCourseOption(idx);
      } else {
        onDeleteCourseOption(trimmed as any);
      }
    }

    // 2. Remove locally from dbCourses state immediately
    setDbCourses(prev => prev.filter(c => c.title?.trim().toLowerCase() !== trimmed.toLowerCase()));

    // 3. Delete from database dbCourses if it exists there
    const matchedDbCourse = dbCourses.find(c => c.title?.trim().toLowerCase() === trimmed.toLowerCase());
    if (matchedDbCourse) {
      try {
        await fetch(`/api/courses/${matchedDbCourse.id}`, { 
          method: 'DELETE',
    ...
    // 4. Reset form subject if matching
    if (formData.subject === trimmed) {
  ...
  const fetchTimetables = () => {
    ...
    Promise.all([
      safeParse('/api/timetables'),
      safeParse('/api/courses')
    ])
      ...
      .catch((err) => console.warn("Notice: Timetables fetch fallback:", err))
  ...
  const conflicts = useMemo(() => {
    ...
    for (let i = 0; i < timetables.length; i++) {
      ...
      for (let j = i + 1; j < timetables.length; j++) {
        ...
        if (areDaysOverlapping(t1.dayOfWeek, t2.dayOfWeek)) {
          ...
          if (overlaps) {
            ...
            if (t1.room && t2.room && t1.room.trim().toLowerCase() === t2.room.trim().toLowerCase()) {
              ...
              const detail = idt(
                `ជាន់បន្ទប់ ${t1.room} (${t1.dayOfWeek} ${t1.startTime}-${t1.endTime} ជាមួយ ${t2.dayOfWeek} ${t2.startTime}-${t2.endTime})`,
                `Room collision ${t1.room} (${t1.dayOfWeek} ${t1.startTime}-${t1.endTime} with ${t2.dayOfWeek} ${t2.startTime}-${t2.endTime})`
              );
            ...
            if (t1.teacherId && t2.teacherId && t1.teacherId === t2.teacherId) {
              ...
              const detail = idt(
                `គ្រូបង្រៀនជាន់ម៉ោង (${t1.dayOfWeek} ${t1.startTime}-${t1.endTime} ជាមួយ ${t2.dayOfWeek} ${t2.startTime}-${t2.endTime})`,
                `Teacher scheduled twice (${t1.dayOfWeek} ${t1.startTime}-${t1.endTime} with ${t2.dayOfWeek} ${t2.startTime}-${t2.endTime})`
              );
  ...
  const filteredTimetables = useMemo(() => {
    return timetables.filter(t => {
      ...
      const matchesDay = !selectedDay || isDayMatching(t.dayOfWeek, selectedDay);
  ...
  const timetablesByDay = useMemo(() => {
    ...
    filteredTimetables.forEach(t => {
      dayOptions.forEach(day => {
        if (isDayMatching(t.dayOfWeek, day)) {
          if (!grouped[day]) grouped[day] = [];
          if (!grouped[day].some(existing => existing.id === t.id)) {
            grouped[day].push(t);
          }
        }
      });
  ...
  const dayDistributionData = useMemo(() => {
    return dayOptions.map(day => {
      const daySlots = timetables.filter(t => t && t.dayOfWeek === day);
  ...
  const conflictAdvisorInsights = useMemo(() => {
    ...
    Object.entries(conflicts).forEach(([id, rawCf]) => {
      const item = timetables.find(t => t && t.id === id);
      if (!item) return;
      ...
      listCf.forEach(cf => {
        ...
        if (cf.type === 'room') {
          ...
          const occupiedRoomsAtThisTime = timetables
            .filter(t => t && t.dayOfWeek === item.dayOfWeek && checkTimeOverlaps(item.startTime, item.endTime, t.startTime, t.endTime))
          ...
          <div className="py-24 text-center px-4">
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-8">
              <button 
                ...
                className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs shadow-lg shadow-blue-600/20 transition-all flex items-center gap-2 cursor-pointer active:scale-95"
              >
