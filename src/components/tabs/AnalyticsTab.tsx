    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.warn("Notice: Analytics response format fallback:", jsonErr);
        }
      } else {
        console.warn(`Notice: Analytics endpoint status ${response.status}`);
      }
      setRawData(fallbackData);
      processPeriodData(fallbackData, period);
    } catch (error) {
      console.warn("Notice: Live analytics fetch fallback:", error);
      setRawData(fallbackData);
      processPeriodData(fallbackData, period);
    } finally {
      setLoading(false);
