...
export default function AnnouncementsTab({ uiLang, students = [] }: { uiLang?: string, students?: any[] } = {}) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [coursesData, annData] = await Promise.all([
      ...
      console.warn("Notice: Announcements fetch fallback:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredTargets = useMemo(() => {
    ...
    const activeStudents = students.filter(s => s && s.status === 'STUDYING');
  ...
  const groupedTargets = useMemo(() => {
    ...
    const courseGroup = courses
      .map(c => {
        const count = activeStudents.filter(s => s && s.course === c.title || s.level === c.title).length;
        return {
          id: c.id,
          label: c.title,
          title: c.title,
