            ...
            {activeTab === "Attendance" && (() => {
              ...
              const autoSaveItemAttendance = async (
                ...
                try {
                  ...
                  const authToken = token || localStorage.getItem("plc_auth_token") || localStorage.getItem("token") || "";
                  ...
                  // Quietly handle offline or network glitches without failing
                  console.warn("Auto-save attendance attempt finished:", err);
                }
              };
              ...
              const handleSaveAttendance = async () => {
                ...
                try {
                  const activeList = isStudentMode 
                    ? students.filter(s => s && s.status === 'STUDYING') 
                    : teachers.filter(t => t && t.status !== 'EXITED' && t.status !== 'STOP' && t.status !== 'RESIGNED' && !isTeacherStopped(t));
              ...
              const allCourses: string[] = Array.from(new Set(students.filter((s: any) => s.status === 'STUDYING').map((s: any) => s.course).filter((c: any): c is string => Boolean(c))));
                          ...
                          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-xl border border-slate-200/80 shadow-3xs shrink-0 select-none">
                            ...
                            <div className="flex items-center gap-1.5 px-2 py-1 border border-slate-200/60 rounded-md bg-white hover:bg-slate-50 hover:border-primary-400 transition-all shadow-3xs cursor-pointer relative">
                              ...
                              <input
                                ...
                                style={{ minHeight: "unset", appearance: "auto", WebkitAppearance: "auto" as any }}
                              />
                            </div>
                        ...
                        <div className="w-full xl:flex-1 bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden flex flex-col relative">
                          ...
                          <div className="px-6 py-1.5 border-b border-slate-200 bg-slate-50/50 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3">
                            ...
                            <div className="flex flex-wrap lg:flex-nowrap items-center gap-2.5 shrink-0">
                              ...
                              {/* Check-Out stats summary */}
                              <div className="bg-white border border-slate-200/80 border-l-[3.5px] border-l-sky-500 rounded-xl p-2 px-3 flex items-center justify-between gap-2 shadow-2xs">
                                <div className="leading-none shrink-0">
                                  <span className="block text-[8px] font-black text-slate-400 uppercase tracking-widest">
                                    📤 {uiLang === "kh" ? "សរុបចេញ • CHECK-OUT" : uiLang === "zh" ? "签退统计 • CHECK-OUT" : "CHECK-OUT SUMMARY"}
                                  </span>
                                  <span className="text-[10.5px] font-black text-sky-600 mt-1 block">
                                    {uiLang === "kh" ? "អត្រា៖ " : uiLang === "zh" ? "比例：" : "Rate: "}
                                    {totalActiveCount > 0 
                                      ? `${uiLang === "kh" ? toKhmerNumeral(((checkOutStats.PRESENT / totalActiveCount) * 100).toFixed(0)) : ((checkOutStats.PRESENT / totalActiveCount) * 100).toFixed(0)}%` 
                                      : (uiLang === "kh" ? "១០០%" : "100%")}
                                  </span>
                                </div>
                                <div className="flex items-center gap-1 text-[9px] font-black">
                                  <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 rounded-md border border-emerald-100">P: {uiLang === "kh" ? toKhmerNumeral(checkOutStats.PRESENT) : checkOutStats.PRESENT}</span>
                                  <span className="px-1.5 py-0.5 bg-rose-50 text-rose-700 rounded-md border border-rose-100">A: {uiLang === "kh" ? toKhmerNumeral(checkOutStats.ABSENT) : checkOutStats.ABSENT}</span>
                                  <span className="px-1.5 py-0.5 bg-amber-50 text-amber-700 rounded-md border border-amber-100">L: {uiLang === "kh" ? toKhmerNumeral(checkOutStats.LATE) : checkOutStats.LATE}</span>
                                  <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded-md border border-blue-100">E: {uiLang === "kh" ? toKhmerNumeral(checkOutStats.PERMISSION) : checkOutStats.PERMISSION}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                                    ...
                                    <div key={item.id} className="bg-white rounded-xl border border-primary-100/60 shadow-3xs p-4 flex flex-col justify-between hover:shadow-2xs transition-all relative overflow-hidden group">
                                      ...
                                      <div className="space-y-3.5 relative z-10">
                                        ...
                                        <div className="flex items-center gap-3">
                                          <div className={`w-11 h-11 rounded-full ${avatarBg} border flex items-center justify-center text-xl font-black shrink-0 relative overflow-hidden`}>
                                            {item.photoUrl || (item as any).photo ? (
                                              <img src={item.photoUrl || (item as any).photo} alt={item.nameKh} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                            ) : (
                                              <span>{isFemale ? "👧" : "👦"}</span>
                                            )}
