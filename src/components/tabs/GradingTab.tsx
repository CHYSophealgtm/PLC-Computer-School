...
export default function GradingTab({ students = [], token, uiLang: propUiLang, courseOptions = [] }: any) {
  ...
  const uniqueSubjects = Array.from(new Set(scores.filter(s => s && s.student && s.student.status === 'STUDYING').map(s => s.subject))).filter(Boolean).sort();
  const uniqueMonths = Array.from(new Set(scores.filter(s => s && s.student && s.student.status === 'STUDYING').map(s => s.month))).filter(Boolean).sort();
  ...
  const passRatePercentage = totalScoresCount > 0 
    ? Math.round((sortedScores.filter(s => s && s.score >= 50).length / totalScoresCount) * 100) 
    : 0;
  ...
  const subjectStats = uniqueSubjects.map(sub => {
    const subScores = sortedScores.filter(s => s && s.subject === sub);
    ...
    const passRate = Math.round((subScores.filter(s => s && s.score >= 50).length / subScores.length) * 100);
  ...
  const aPassRate = aTotalCount > 0 
    ? Math.round((analyticsScores.filter(s => s && s.score >= 50).length / aTotalCount) * 100) 
    : 0;
  ...
  const aSubjectStats = (analyticsSubjectFilter === "all" ? uniqueSubjects : [analyticsSubjectFilter]).map(sub => {
    const subScores = scores.filter(s => s && s.student && s.student.status === 'STUDYING' && s.subject === sub && (analyticsMonthFilter === "all" || s.month === analyticsMonthFilter));
    if (subScores.length === 0) return null;
    const avg = Number((subScores.reduce((sum, s) => sum + s.score, 0) / subScores.length).toFixed(1));
    const highest = Math.max(...subScores.map(s => s.score));
    const passRate = Math.round((subScores.filter(s => s && s.score >= 50).length / subScores.length) * 100);
