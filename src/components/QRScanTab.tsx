...
import { Smartphone, ArrowDown, ArrowUp, GraduationCap, User, Search, SlidersHorizontal, List, LayoutGrid, Clock, Camera, Check, X, BookOpen, Hourglass, Sparkles, Settings2, Edit3, Save, Undo2, UserCheck, Users, ChevronDown, Calendar, ChevronLeft, ChevronRight, RefreshCw, AlertCircle, Video } from 'lucide-react';
import { Student as StudentType } from "../types";
import { toKhmerNumeral, getKhmerDateString, getKhmerTimeString } from "../lib/khmerUtils";
...
export default function QRScanTab({
  ...
  const configRuleOptions = useMemo(() => {
    ...
    // Add teachers
    teachers.forEach(t => {
      if (t && t.status !== "SUSPENDED" && t.status !== "EXITED") {
        const shiftInfo = getShiftAndHours(t.shift, true, t);
        list.push({
          id: t.id,
          displayId: t.teacherId,
          name: t.nameKh || t.name || t.nameEn || "",
          nameEn: t.nameEn || "",
          type: "teacher",
  ...
  const currentRulePersonFullDetails = useMemo(() => {
    ...
    const person = isTeacher
      ? teachers.find(t => t && t.id === currentRulePerson.id || t.teacherId === currentRulePerson.displayId)
      : students.find(s => s && s.id === currentRulePerson.id || s.studentId === currentRulePerson.displayId);
  ...
  const selectedPersonFullDetails = useMemo(() => {
    ...
    const person = isTeacher
      ? teachers.find(t => t && t.id === selectedScanPerson.id || t.teacherId === selectedScanPerson.displayId)
      : students.find(s => s && s.id === selectedScanPerson.id || s.studentId === selectedScanPerson.displayId);
    return {
      person,
      shiftInfo: getShiftAndHours(person?.shift, isTeacher, person)
    };
  ...
  // Helper to extract student/teacher ID from scanned text or URL
  const parseScannedId = (text: string): string => {
    if (!text) return "";
    let clean = text.trim();
    
    // Check URL parameters
    if (clean.includes("portal_student=")) {
      try {
        const urlObj = new URL(clean);
        const val = urlObj.searchParams.get("portal_student");
        if (val) return val.trim();
      } catch (e) {
        const match = clean.match(/portal_student=([^&]+)/);
        if (match) return match[1].trim();
      }
    }
    if (clean.includes("student=")) {
      try {
        const urlObj = new URL(clean);
        const val = urlObj.searchParams.get("student");
        if (val) return val.trim();
      } catch (e) {
        const match = clean.match(/student=([^&]+)/);
        if (match) return match[1].trim();
      }
    }
    if (clean.includes("id=")) {
      try {
        const urlObj = new URL(clean);
        const val = urlObj.searchParams.get("id");
        if (val) return val.trim();
      } catch (e) {
  ...
  // Helper to match scanned string against members
  const matchPersonByScannedId = (scannedId: string) => {
    if (!scannedId) return null;
    const target = scannedId.trim().toLowerCase();

    // 1. Exact match on displayId or id
    const exact = scanOptions.find(o => 
      (o.displayId && o.displayId.trim().toLowerCase() === target) ||
      (o.id && o.id.trim().toLowerCase() === target)
    );
    if (exact) return exact;

    // 2. Partial match on displayId or id
    const partial = scanOptions.find(o => 
      (o.displayId && (o.displayId.toLowerCase().includes(target) || target.includes(o.displayId.toLowerCase()))) ||
      (o.id && (o.id.toLowerCase().includes(target) || target.includes(o.id.toLowerCase())))
    );
    if (partial) return partial;

    // 3. Match on name or nameEn
    const nameMatch = scanOptions.find(o => 
      (o.name && o.name.toLowerCase() === target) ||
      (o.nameEn && o.nameEn.toLowerCase() === target)
    );
    return nameMatch || null;
  };

  // Core attendance scan function for both manual triggers and camera QR code scans
  const processAttendanceScan = (targetPersonInput?: any, overrideScanMode?: "check-in" | "check-out"): boolean => {
    const person = targetPersonInput || selectedScanPerson;
    if (!person) {
      showToast(
        lang === "kh" 
    ...
    // Determine person full details for shift hours lookup
    const isTeacher = person.type === "teacher";
    const rawPerson = isTeacher
      ? teachers.find(t => t && t.id === person.id || t.teacherId === person.displayId)
      : students.find(s => s && s.id === person.id || s.studentId === person.displayId);
    const shiftInfo = getShiftAndHours(rawPerson?.shift, isTeacher, rawPerson);

    // Determine scan type (Check-In vs Check-Out)
    const effectiveScanMode = overrideScanMode || scanMode;
    let isCheckIn = effectiveScanMode === "check-in";

    // Validate that current time is within study/work hours
    const hoursStr = shiftInfo?.hours || "---";
    const validation = isTimeValidForShift(person.id, hoursStr, isCheckIn, currentHour, currentMinute);
    ...
    if (isAlreadyScanned) {
      ...
      showToast(
        lang === "kh" 
          ? `បរាជ័យ៖ ${person.name} បាន${modeText}រួចរាល់ហើយសម្រាប់ថ្ងៃនេះ!` 
          : lang === "zh" 
            ? `失败：${person.name} 今天已经完成${modeText}！` 
            : `Failed: ${person.name} has already ${modeText} for today!`, 
        "error"
      );
      return false;
    }
    ...
    if (person?.id && individualRules[person.id]) {
      const customRule = individualRules[person.id];
    ...
    const newLog = {
      ...
      studentId: person.displayId, // Store correct formatted display ID
      dbId: person.id, // Store DB ID for state mapping
      status: statusText,
      statusKh: statusKhText,
      course: person.details,
      time: timeString,
      itemType: person.type,
      date: selectedDate
    };
    ...
    if (isCheckIn) {
      ...
      showToast(
        lang === "kh" 
          ? `ស្កេនចូលជោគជ័យ៖ ${person.name} (${statusLabelForToast})` 
          : lang === "zh" 
            ? `签到成功：${person.name} (${statusLabelForToast})` 
            : `Check-in successful: ${person.name} (${statusLabelForToast})`, 
      ...
      showToast(
        lang === "kh" 
          ? `ស្កេនចេញជោគជ័យ៖ ${person.name} (${outStatusLabelForToast})` 
          : lang === "zh" 
            ? `签退成功：${person.name} (${outStatusLabelForToast})` 
            : `Check-out successful: ${person.name} (${outStatusLabelForToast})`, 
        statusText === "EARLY_LEAVE" ? "info" : "success"
      );
    }

    // Trigger automatic Telegram Bot notification dispatch
    try {
      const typeLabel = person.type === 'teacher' ? '👨‍🏫 គ្រូបង្រៀន/បុគ្គលិក' : '🎓 សិស្ស';
      const actionIcon = isCheckIn 
        ? (statusText === 'LATE' ? '⏰ ស្កេនចូល (យឺត)' : '✅ ស្កេនចូល (ទាន់ពេល)')
        : (statusText === 'EARLY_LEAVE' ? '⚠️ ស្កេនចេញ (មុនម៉ោង)' : '🚪 ស្កេនចេញ');

      const telegramMsg = `<b>${actionIcon}</b>\n\n` +
        `👤 <b>${typeLabel}:</b> ${person.name} (${person.nameEn || 'N/A'})\n` +
        `🆔 <b>អត្តសញ្ញាណ:</b> <code>${person.displayId}</code>\n` +
        `📚 <b>ព័ត៌មាន:</b> ${person.details || 'N/A'}\n` +
        `🕒 <b>ម៉ោង:</b> ${timeString}\n` +
        `📅 <b>កាលបរិច្ឆេទ:</b> ${selectedDate}\n\n` +
        `🏫 <i>ប្រព័ន្ធគ្រប់គ្រងសាលារៀន PLC Academy</i>`;
  ...
  // Handle simulated manual scan trigger
  const handleSimulatedScan = () => {
    processAttendanceScan(selectedScanPerson);
  };

  // Real Camera Scanner Initialization via Html5Qrcode
  useEffect(() => {
    let isMounted = true;
    ...
    const startCamera = async () => {
      ...
      // Wait a short delay for DOM element `#qr-camera-reader` to mount
      ...
      try {
        ...
        try {
          ...
          if (fetchedDevices && fetchedDevices.length > 0) {
            devices = fetchedDevices.map((d, i) => ({
              id: d.id,
              label: d.label || `${lang === "kh" ? "កាមេរ៉ាទី" : "Camera"} ${i + 1}`
            }));
            if (isMounted) setCameraDevices(devices);
          }
        } catch (devErr) {
          console.warn("Could not query cameras list:", devErr);
        }

        // Selected camera device ID or environment back camera
        let targetCameraConfig: any = { facingMode: "environment" };
        if (selectedCameraId) {
          targetCameraConfig = selectedCameraId;
          ...
          targetCameraConfig = chosenId;
          if (isMounted) setSelectedCameraId(chosenId);
        }

        const boxWidth = Math.min(window.innerWidth - 60, 240);

        await qrScanner.start(
          targetCameraConfig,
          {
            fps: 10,
            qrbox: { width: boxWidth, height: boxWidth },
            aspectRatio: 1.0
          },
          (decodedText) => {
            if (!isMounted) return;

            // Throttle duplicate scans within 3 seconds
            const now = Date.now();
            ...
            const extractedId = parseScannedId(decodedText);
            setLastScannedResult(extractedId || decodedText);

            // Match member in current option list
            const matchedPerson = matchPersonByScannedId(extractedId);
            if (matchedPerson) {
              setSelectedScanStudentId(matchedPerson.id);
              const success = processAttendanceScan(matchedPerson);
              if (success) {
                setIsScanSuccessFlash(true);
                setTimeout(() => setIsScanSuccessFlash(false), 1500);
              }
            } else {
              showToast(
                lang === "kh" 
                  ? `រកមិនឃើញទិន្នន័យសមាជិក ID: ${extractedId}` 
                  : lang === "zh" 
                    ? `未在系统中找到 ID 为 ${extractedId} 的人员` 
                    : `No matching member found for ID: ${extractedId}`,
                "error"
              );
            }
          },
          () => {
            // Frame decode failures are expected when no QR is present in viewfinder
          }
        );
        ...
        if (
          ...
          // Benign media play interruption during unmount or restart
          return;
        }
        ...
        if (errMsg.includes("Permission") || errMsg.includes("NotAllowedError") || errMsg.includes("denied")) {
          setCameraError(
            lang === "kh" 
              ? "មិនមានសិទ្ធិចូលប្រើកាមេរ៉ា! សូមចុច Allow Camera Permission នៅក្នុង Browser" 
              : lang === "zh" 
                ? "无相机访问权限！请在浏览器中允许相机权限" 
                : "Camera permission denied! Please allow camera access in browser settings"
          );
        } else {
          setCameraError(
            lang === "kh" 
              ? "មិនអាចបើកកាមេរ៉ាបានទេ៖ សូមពិនិត្យថាតើកាមេរ៉ាកំពុងត្រូវបានប្រើប្រាស់ដោយកម្មវិធីផ្សេង ឬភ្ជាប់កាមេរ៉ាឡើងវិញ" 
              : lang === "zh" 
                ? "无法启动相机：请检查相机是否被其他程序占用" 
                : "Unable to start camera: Please verify camera connection or permissions"
          );
        }
      }
    };
  ...
  // Toggle Camera Module Activation
            ...
            <div className="flex flex-wrap items-stretch justify-start gap-4">
              <AnimatePresence mode="popLayout">
                {filteredLogs.map((log, index) => {
                  ...
                  // Lookup the actual student or teacher details to capture shift and study hours
                  const person = isTeacher
                    ? teachers.find(t => t && t.id === log.dbId || t.teacherId === log.studentId)
                    : students.find(s => s && s.id === log.dbId || s.studentId === log.studentId);
        ...
        <div className={isSessionFeedHidden ? "w-full max-w-none grid grid-cols-1 lg:grid-cols-[1.7fr_2.3fr] xl:grid-cols-[1.7fr_2.3fr] gap-6 items-stretch lg:min-h-[580px] xl:min-h-[660px] 2xl:min-h-[760px] mt-2" : "space-y-6 xl:col-span-1"}>
          ...
          <div id="live-camera-feed-card" className="bg-white border border-slate-200/90 rounded-[28px] p-5 shadow-sm flex flex-col h-full">
            ...
            <div className="relative flex-1 min-h-[320px] lg:min-h-[360px] mb-5 rounded-2xl bg-black border border-slate-800 overflow-hidden flex flex-col items-center justify-center text-center p-4">
              {isScanning ? (
                <>
                  {/* Container for html5-qrcode camera element */}
                  <div 
                    id="qr-camera-reader" 
                    className="absolute inset-0 w-full h-full object-cover [&_video]:w-full [&_video]:h-full [&_video]:object-cover [&_video]:rounded-2xl"
                  />

                  {/* Visual success flash overlay */}
                  {isScanSuccessFlash && (
                    <div className="absolute inset-0 z-30 bg-emerald-500/30 border-4 border-emerald-500 rounded-2xl flex items-center justify-center backdrop-blur-3xs animate-in fade-in duration-200">
                      <div className="bg-emerald-600 text-white px-4 py-2 rounded-xl font-black text-xs shadow-lg flex items-center gap-2">
                        <Check className="w-5 h-5 text-white" />
                        <span>{lang === "kh" ? "ស្កេនវត្តមានជោគជ័យ!" : lang === "zh" ? "扫码成功！" : "Scan Success!"}</span>
                      </div>
                    </div>
                  )}

                  {/* Overlay Scanner Brackets & Laser Line when camera is active and no error */}
                  {!cameraError && (
                    <>
                      <div className="absolute top-4 left-4 w-5 h-5 border-t-2 border-l-2 border-emerald-400 z-10"></div>
                      <div className="absolute top-4 right-4 w-5 h-5 border-t-2 border-r-2 border-emerald-400 z-10"></div>
                      <div className="absolute bottom-4 left-4 w-5 h-5 border-b-2 border-l-2 border-emerald-400 z-10"></div>
                      <div className="absolute bottom-4 right-4 w-5 h-5 border-b-2 border-r-2 border-emerald-400 z-10"></div>

                      {/* Red Laser Scanning Line */}
                      <div className="absolute left-4 right-4 h-0.5 bg-red-500 shadow-[0_0_12px_#ef4444] animate-[bounce_2s_infinite] z-10 pointer-events-none"></div>

                      {/* Active Status Badge at Top Left */}
                      <div className="absolute top-3 left-3 z-10 flex items-center gap-1.5 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/20">
                        ...
                        <span className="text-[9px] font-black text-emerald-300 uppercase tracking-widest">
                          {lang === "kh" ? "កាមេរ៉ាកំពុងស្កេន" : lang === "zh" ? "镜头扫描中" : "CAMERA ACTIVE"}
                        </span>
                      </div>

                      {/* Camera Selector Dropdown (if multiple devices found) */}
                      {cameraDevices.length > 1 && (
                        <div className="absolute top-3 right-3 z-20">
                          <select
                            value={selectedCameraId}
                            onChange={(e) => setSelectedCameraId(e.target.value)}
                            className="bg-black/70 text-white text-[9px] font-bold border border-white/30 rounded-lg px-2 py-1 focus:outline-none cursor-pointer"
                          >
                      ...
                      {/* Bottom Instructions / Last Result */}
                      <div className="absolute bottom-3 inset-x-3 z-10 text-center bg-black/60 backdrop-blur-md p-2 rounded-xl border border-white/10">
                        {lastScannedResult ? (
                          <div className="text-[10px] font-black text-emerald-400 truncate">
                            {lang === "kh" ? "ស្កេនចុងក្រោយ៖ " : "Last Scanned: "}{lastScannedResult}
                          </div>
                        ) : (
                          <div className="text-[9px] text-slate-300 font-bold">
                            {lang === "kh" ? "តម្រង់កូដ QR/Barcode លើប័ណ្ណសម្គាល់អោយចំកណ្តាលកាមេរ៉ា..." : lang === "zh" ? "将二维码/条形码对准镜头..." : "Align QR or Barcode ID card with the camera..."}
                          </div>
                        )}
                      </div>
                    </>
                  ...
                  {/* Camera Error Display with Retry & Simulate Fallback */}
                  {cameraError && (
                    <div className="absolute inset-0 z-30 bg-slate-950/90 flex flex-col items-center justify-center p-5 text-center">
                      <div className="w-12 h-12 rounded-2xl bg-red-500/20 border border-red-500/40 flex items-center justify-center text-red-400 mb-3">
                        <AlertCircle className="w-6 h-6" />
                      </div>
                      <div className="text-xs font-black text-white mb-1">
                        {lang === "kh" ? "មិនអាចចូលប្រើកាមេរ៉ាបានទេ" : lang === "zh" ? "无法打开相机" : "Camera Access Failed"}
                      </div>
                      <div className="text-[10px] text-slate-300 font-medium max-w-xs mb-4 leading-relaxed">
                        {cameraError}
                      </div>
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3.5 py-1.5 bg-primary-600 hover:bg-primary-500 text-white rounded-xl text-[10px] font-black flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
                        >
                          <RefreshCw className="w-3.5 h-3.5" />
                          {lang === "kh" ? "ព្យាយាមម្តងទៀត" : lang === "zh" ? "重试" : "Try Again"}
                        </button>
                        ...
                        <button
                          ...
                          className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[10px] font-black flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
                        >
                          <Check className="w-3.5 h-3.5" />
                          {lang === "kh" ? "ស្កេនសាកល្បង (Test Scan)" : lang === "zh" ? "测试刷卡" : "Test Auto Scan"}
                        </button>
                      </div>
                    </div>
                  )}
