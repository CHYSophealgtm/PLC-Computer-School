...
export default function AssetsTab({ uiLang, showToast }: AssetsTabProps) {
      ...
      onConfirm: async () => {
        try {
          ...
          if (sale) {
            ...
            setSales(prev => prev.filter(s => s && s.id !== saleId));
  ...
  const handleExportCSV = () => {
    if (viewMode === "sales") {
      ...
      const data = filteredSales.filter(Boolean).map((row, idx) => ({
        ...
        "ប្រភេទ": row?.category || "-",
        "ចំនួនលក់": row?.quantitySold || 0,
        "តម្លៃលក់រាយ": `${(row?.pricePerUnit || 0).toFixed(2)}`,
        "ចំណូលសរុប": `${(row?.totalRevenue || 0).toFixed(2)}`,
      ...
      const data = filteredAssets.filter(Boolean).map(row => ({
        "លេខកូដ": row.id,
        "ឈ្មោះសម្ភារៈ": row?.nameKh || "",
        "ប្រភេទ": row?.category || "-",
        "ចំនួន": row?.quantity || 0,
        "តម្លៃរាយ": `${(row?.unitPrice || 0).toFixed(2)}`,
        "តម្លៃសរុប": `${((row?.quantity || 0) * (row?.unitPrice || 0)).toFixed(2)}`,
        "ទីតាំង": row?.location || "-",
              ...
              <form onSubmit={handleSellAssetSubmit} className="p-6 space-y-4">
                ...
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                  <div className="space-y-1.5">
                    ...
                    >
                      {assets.filter(a => a && (a.quantity > 0 || a.id === sellAsset?.id)).map(a => {
