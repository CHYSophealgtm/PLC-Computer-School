...
export default function SubjectDropdown({ value, onChange, allowManage = true }: SubjectDropdownProps) {
  ...
  const selectedObj = subjectsList.find(s => s && (s.id === value || s.nameKh === value || s.nameEn === value)) || {
  ...
  const filtered = subjectsList.filter(s => s && (
  ...
  const handleDeleteSubject = (idToDelete: string) => {
    ...
    const updated = subjectsList.filter(s => s && s.id !== idToDelete && s.nameKh !== idToDelete);
    setSubjectsList(updated);
    saveAllSubjects(updated);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {allowManage && (
                        <div className="opacity-80 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
