...
export async function exportToExcel(data: any[], filename: string, title?: string) {
  ...
  if (title) {
    ...
    titleCell.font = { name: 'Khmer OS Muol Light', size: 14, bold: true, color: { argb: 'FF1e3a8a' } };
  ...
  headerRow.eachCell((cell) => {
    cell.font = { name: 'Khmer OS Battambang', bold: true, color: { argb: 'FFffffff' }, size: 11 };
    cell.fill = {
      type: 'pattern',
      pattern: 'solid',
  ...
  data.forEach((rowObj, index) => {
    ...
    row.eachCell((cell, colNumber) => {
      cell.font = { name: 'Khmer OS Battambang', size: 10, color: { argb: 'FF1e293b' } };
